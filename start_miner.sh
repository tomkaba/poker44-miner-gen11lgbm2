...
for raw_id in $(echo "$IDS_STRING" | tr ',' '\n'); do
  ...
  screen -dmS "$SESSION" /bin/bash -c "
    ...
    export POKER44_SINGLE_HAND_MODEL_ALIAS=gen11lgbm2
    export POKER44_CHUNK_SCORER=gen11lgbm2
    export POKER44_GEN11LGBM2_PROFILE=$REPO/models/benchmark_lgbm_profile.json
